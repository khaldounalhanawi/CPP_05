...
/*   Updated: 2026/04/26 17:48:34 by kalhanaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	Intern		intern;
	Bureaucrat	employee ("normal", 140);
	Bureaucrat	Boss ("goodman", 1);
	AForm		*myhome;

	myhome = intern.makeForm ("mistake", "home");
	myhome = intern.makeForm ("shrubbery creation", "home");

	employee.signForm(*myhome);
	employee.executeForm (*myhome);

	Boss.executeForm (*myhome);
	delete myhome;
}
...
{
	int	index;
	void (*tests[])() = {test0, test1, test2, test3, test4};
